...
def draw(*args):
    ...
    if game.blocks_gone:
        new_game()    
    if p5.keyIsPressed:
        if (p5.keyCode == RIGHT_ARROW) and (not game.game_over):  # Move right
            game.paddle.x = min(game.paddle.x + 12, WIDTH - P_WIDTH) 
        elif (p5.keyCode == LEFT_ARROW) and (not game.game_over): # Move left
...
class Ball(object):
    def __init__(self,level=1):
        # ball velocity
        self.vel = createVector(3 + level,-3 - level)
